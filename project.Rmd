...
Como queremos gerar recomendações a um grupo que vai tirar melhor proveito delas, retiramos utilizadores com mais de 32 ratings. E, de forma a ter contexto suficiente para poder gerar recomendações relevante, definimos que cada utilizador tem que ter pelo menos 10 classificações.


Os nossos dados processados são os seguintes
```{r echo=FALSE}
...
Sem levar em conta as caracteristicas das pessoas, criamos a matrix binária correlacionando os utilizadores e os filmes.
...
Para gerar recomendações de filmes, usamos 4 algoritmos: Popularidade, Regras de Associação, User-Based CF, Item-Based CF.
Para as regras de associação usámos um suporte de 0.01, pois evitar a escassez de regras. Mas definimos a confiança a 0.8 para as garantir que as regras geradas fossem fiáveis. Definimos também o comprimento máximo das regras a 3, porque temos um mínimo de 10 filmes avaliados por pessoa e vão ser recomendados no máximo 5 filmes.
Nos algoritmos UBCF e IBCF, definimos os nearest-neighbours a 5. Não escolhemos um número mais alto para os algoritmos não fazerem recomendações com vizinhos que, na verdade, não são assim tão semelhantes.
...
### Prever os ratings
Para prever as avaliações dos filmes de utilizadores usámos apenas UBCF e IBCF, visto que a popularidade e as regras de associação não são algoritmos válidos para esta tarefa. Os parametros dos algoritmos são os mesmos que os usados na recomendação de filmes pelas mesmas razões. Segue-se a matrix de avaliações para um conjunto limitado de pessoas e filmes:

```{r echo=FALSE}
rm <- as(as.data.frame(ratings), "realRatingMatrix")
plot(image(rm[100:200,100:200], cuts=8, colorkey=T, col=topo.colors(10)))
```


Para fazer a avalição dos modelos gerados, definimos que o conceito de uma boa pontuação é um valor maior ou igual a 3,0. Os dados de treino, classificações dadas e parâmetros dos modelos são iguais aos da recomendações de filmes. 
...
## Com contexto
Nesta secção, filtramos os dados usados nos modelos às categorias mais predominantes de cada género de pessoa. No nosso caso, estas categorias são os utilizadores com 15 a 20 anos em ambos os géneros. Também experimentámos apenas para a idade equivalente a 15 a 20 anos. Os parametros usados são os mesmos da secção anterior, sendo que a única diferença é de facto o contexto dos dados. Assim, o objetivo desta secção é observar se existe mudança na *performance* dos modelos com esta alteração.
### Recomendações de filmes

Primeiro, observamos para a idade mais predominante, os 15 a 20 anos. Os resultados são:

```{r echo=FALSE}
users <- filter(profiles, agegroup == 15 )$userid
bm <- as(as.data.frame(filter(ratings, userid %in% users)), "binaryRatingMatrix")

l <- eval_real_func(bm, 0.80, 5, 5)
par(mfrow=c(1,2))
plot(l$res,"ROC" ,annotate=c("1","2","3"))
plot(l$res,"prec/rec" ,annotate=c("1","2","3"))
...
Aqui, para pessoas do género feminino é com a idade entre 15 a 20 anos:
```{r echo=FALSE}
...
Finalmente, para os utilizadores masculinos de 15 a 20 anos:

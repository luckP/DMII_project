...
```{r}
#ratings <- filter(ratings, userid %in% profiles$userid) %>% mutate(year = substr(date, 1, 4))
```
...
valid_movies <- filter(valid_movies, n >= 150)
...
#bm <- as(filter(ratings, year == 2009),"binaryRatingMatrix")
#es <- evaluationScheme(bm, method="split", train=0.95, given=1)

param <- list(supp = 0.02, conf = 0.8, maxlen = 20)

#models <- list(
  #pop_model = list(name = "POPULAR"),
  #ar_model = list(name = "AR", param = param))


```

```{r}
#pop_model <- Recommender(bm[ bm@data@itemsetInfo$itemsetID %in% train$userid ,], "POPULAR")
pop_model <- Recommender(bm[ bm@data@itemsetInfo$itemsetID %in% train$userid ,], "AR", param)
...
```{r}
#pop_model <- Recommender(as(train,"binaryRatingMatrix"), "POPULAR")
recs <- predict(pop_model, 
                sample(bm[ bm@data@itemsetInfo$itemsetID %in% test$userid ,],500),
                type="topNList", 
                n=15)
calcPredictionAccuracy(recs, 
                       sample(bm[ bm@data@itemsetInfo$itemsetID %in% test$userid ,],500),
                       given=15)
```
...
```{r}
res <- evaluate(x = es, method = models, n = 1:3)
```
...
ar_model <- Recommender(getData(es, "train"), "AR", param)
...
recs <- predict(pop_model, getData(es, "unknown"), type="topNList", n=5)
calcPredictionAccuracy(recs, getData(es, "unknown"), byUser = TRUE, given=5)

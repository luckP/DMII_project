...
# Sitemas de Recomendação
Explicar esta funcao
...
Para as regras de associação usámos um suporte de 0.01, pois evitar a escassez de regras. Mas definimos a confiança a 0.8 para as garantir que as regras geradas fossem fiáveis. Definimos também o comprimento máximo das regras a 5, porque temos um mínimo de 10 filmes avaliados por pessoa e vão ser recomendados no máximo 5 filmes.
...
image(rm[100:200,100:200], cuts=8, colorkey=T, col=topo.colors(10))
```
...
```{r}
piggy <- sample((filter(ratings, !(userid %in% l$eval@runsTrain)))$userid,1)
user_movies <- (filter(ratings, userid == piggy)$movieid)
user_movies <- as.character(user_movies)
activeuser <- matrix(0, 1, ncol=length(unique(ratings$movieid)), 
                      dimnames = list(piggy, unique(ratings$movieid)))
